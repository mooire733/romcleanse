...
- 对话中只回传「判读结论 + 异常项包名」，**不粘贴完整包列表、完整 settings 输出、完整白名单**——包名本身能暴露健康/金融/亲密类应用，全量数据进 LLM 上下文就是泄露面
- 全量原始输出只落盘到本机（与快照同目录），报告给路径不给全文
- 禁止读取通知**内容**、短信、账号数据——本审计只读设置与包元数据
- 审计结果属于用户隐私：未经用户明确要求，不写入任何外部仓库/日志服务
